...
def covid19_init_SC(location,
    ...
    fab eagle_vecma covid19_init_SC:location=brent,nb_thread=4,PJ=True
    ============================================================================
    '''
    ...
    if os.path.isfile(log_submission_csv_file) == False:
        with open(log_submission_csv_file, 'a+') as f:
            f.write("nb_thread,total elapsed jobs "
                    "submission time (in second)\n")
            f.flush()
    ...
    campaign.save_state(os.path.join(work_dir_SCSampler,
                                     "campaign_state.json")
                        )
    sampler.save_state(os.path.join(work_dir_SCSampler,
...
def covid19_analyse_SC(location, ** args):
    ...
    for output_column in ["dead",
        ...
        # read json file
        with open(os.path.join(work_dir_SCSampler,
                               "campaign_state.json"
                               ), "r") as infile:
        ...
        output_file_name = os.path.join(
            work_dir_SCSampler,
            'plot_first_order_sobol_index[%s].png' % (output_column)
        )
        print(output_file_name)
        plt.savefig(output_file_name, dpi=400)
