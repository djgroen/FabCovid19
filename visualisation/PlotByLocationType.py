...
def plot(df, output_dir, locations):
    ...
    fig = make_subplots(rows=9, cols=1, shared_xaxes=True, vertical_spacing=0.01, horizontal_spacing=0.05)
    row=1
    for location in enumerate(locations):
        subdf = df[df['type']==location[1]]
        fig.add_trace(go.Scatter(x=subdf['time'], y=subdf['mean'], name=location[1],  line=dict(color=colors[row])), row=row, col=1)
        row+=1
    ...
    fig.update_layout(legend_orientation="h",  autosize=True, width=800, height=1000, font=dict(family="Courier New, monospace",size=12))
